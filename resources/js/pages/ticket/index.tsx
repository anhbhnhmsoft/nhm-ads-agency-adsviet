...
export default function TicketIndex({ tickets, error }: TicketPageProps) {
    ...
    const isCustomerOrAgency = checkRole([
        _UserRole.CUSTOMER,
        _UserRole.AGENCY,
    ]);
    ...
    const getCurrentTab = () => {
        ...
        if (
            urlString.includes('filter[status_not_in]') ||
            urlString.includes('status_not_in')
        ) {
            return 'pending';
        }
    ...
    const [activeTab, setActiveTab] = useState<'pending' | 'resolved' | 'all'>(
        getCurrentTab(),
    );
    ...
    const handleDeleteTicket = (ticketId: string) => {
        if (
            !confirm(
                t('ticket.confirm_delete', {
                    defaultValue: 'Bạn có chắc muốn xóa yêu cầu này?',
                }),
            )
        ) {
            return;
        ...
        router.delete(ticket_destroy(ticketId).url, {
            ...
            onSuccess: () => {
                toast.success(
                    t('ticket.delete_success', {
                        defaultValue: 'Xóa yêu cầu thành công',
                    }),
                );
            },
            onError: (errors) => {
    ...
    const displayTickets = (tickets?.data || []).filter(
        (ticket: Ticket) => !deletedTicketIds.has(ticket.id),
    );
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">
                            {t('ticket.title', { defaultValue: 'Hỗ trợ' })}
                        </h1>
                        <p className="text-muted-foreground">
                            {t('ticket.list', {
                                defaultValue: 'Danh sách yêu cầu hỗ trợ',
                            })}
                        </p>
                    </div>
                    {!isAdmin && (
                        <Button onClick={() => setShowCreateDialog(true)}>
                            ...
                            {t('ticket.create', {
                                defaultValue: 'Tạo yêu cầu hỗ trợ',
                            })}
                        </Button>
                    )}
                </div>
                ...
                >
                    <TabsList>
                        <TabsTrigger value="pending">
                            {t('ticket.tabs.pending', {
                                defaultValue: 'Cần xử lý',
                            })}
                        </TabsTrigger>
                        <TabsTrigger value="resolved">
                            {t('ticket.tabs.resolved', {
                                defaultValue: 'Đã giải quyết',
                            })}
                        </TabsTrigger>
                        ...
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {t('ticket.create_description', {
                                    defaultValue:
                                        'Mô tả vấn đề của bạn để nhận được hỗ trợ',
                                })}
                            </DialogDescription>
                        </DialogHeader>
                        <form
                        ...
                        >
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder={t(
                                        'ticket.subject_placeholder',
                                        { defaultValue: 'Nhập chủ đề...' },
                                    )}
                                    required
                                />
                            ...
                            {!isCustomerOrAgency && (
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            <SelectItem
                                                value={_TicketPriority.LOW.toString()}
                                            >
                                                {t('ticket.priority.low')}
                                            </SelectItem>
                                            <SelectItem
                                                value={_TicketPriority.MEDIUM.toString()}
                                            >
                                                {t('ticket.priority.medium')}
                                            </SelectItem>
                                            <SelectItem
                                                value={_TicketPriority.HIGH.toString()}
                                            >
                                                {t('ticket.priority.high')}
                                            </SelectItem>
                                            <SelectItem
                                                value={_TicketPriority.URGENT.toString()}
                                            >
                                                {t('ticket.priority.urgent')}
                                            </SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    placeholder={t(
                                        'ticket.description_placeholder',
                                        {
                                            defaultValue:
