...
import { EmployeeForAssignment, Manager } from '@/pages/user/types/type';
import {
    user_assign_employee,
    user_get_employees_by_manager,
    user_unassign_employee,
} from '@/routes';
    ...
    const loadEmployees = async () => {
        ...
        try {
            const response = await fetch(
                user_get_employees_by_manager({ managerId: selectedManagerId })
                    .url,
            );
    ...
    return (
        <div className="space-y-4">
            <div className="grid gap-2">
                <label>
                    {t('user.select_manager', { defaultValue: 'Chọn quản lý' })}
                </label>
                <Select
                ...
                >
                    <SelectTrigger>
                        <SelectValue
                            placeholder={t('user.select_manager_placeholder', {
                                defaultValue: 'Chọn quản lý...',
                            })}
                        />
                    </SelectTrigger>
                    <SelectContent>
                    ...
                    <div className="space-y-2">
                        <h3 className="font-semibold">
                            {t('user.employee_list', {
                                defaultValue: 'Danh sách nhân viên',
                            })}
                        </h3>
                        {loading ? (
                            <div>
                            ...
                            <div className="text-gray-500">
                                {t('user.no_employees', {
                                    defaultValue: 'Không có nhân viên nào',
                                })}
                            </div>
                        ) : filteredEmployees.length === 0 ? (
                            <div className="text-gray-500">
                                {t('user.no_employees_found', {
                                    defaultValue:
