...
export default function SpendReportIndex({
    ...
    const breadcrumbs: IBreadcrumbItem[] = useMemo(
        () => [
            {
                title: t('spend_report.title', {
                    defaultValue: 'Báo cáo chi tiêu',
                }),
    ...
    const datePresetOptions: DatePresetOption[] = [
        {
            value: 'last_7d',
            label: t('spend_report.preset_7d', { defaultValue: '7 ngày qua' }),
        },
        {
            value: 'last_14d',
            label: t('spend_report.preset_14d', {
                defaultValue: '14 ngày qua',
            }),
        },
        {
            value: 'last_28d',
            label: t('spend_report.preset_28d', {
                defaultValue: '28 ngày qua',
            }),
        },
        {
            value: 'last_30d',
            label: t('spend_report.preset_30d', {
                defaultValue: '30 ngày qua',
            }),
        },
        {
            value: 'last_90d',
            label: t('spend_report.preset_90d', {
                defaultValue: '90 ngày qua',
            }),
        },
    ];
    ...
    const percentChange = useMemo(() => {
        ...
        const firstHalf = insightData.chart
            .slice(0, midPoint)
            .reduce((acc, item) => acc + item.value, 0);
        const secondHalf = insightData.chart
            .slice(midPoint)
            .reduce((acc, item) => acc + item.value, 0);
    ...
    const pieData = useMemo(() => {
        if (
            !reportData?.account_spend ||
            reportData.account_spend.length === 0
        ) {
            return [];
        }
        const total = reportData.account_spend.reduce(
            (acc, item) => acc + item.amount_spent,
            0,
        );
        return reportData.account_spend.map((item, index) => {
            const displayName =
                item.account_name && item.account_name.trim().length > 0
                    ? item.account_name
                    : item.account_id ||
                      t('spend_report.unknown_account', {
                          defaultValue: 'Tài khoản chưa xác định',
                      });
            return {
                name: displayName,
                ...
                percent:
                    total > 0
                        ? Math.round((item.amount_spent / total) * 100)
                        : 0,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4 md:p-6">
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">
                            {t('spend_report.title', {
                                defaultValue: 'Báo cáo chi tiêu',
                            })}
                        </h1>
                        <p className="text-muted-foreground">
                            {t('spend_report.subtitle', {
                                defaultValue:
                                    'Theo dõi chi tiêu quảng cáo của bạn',
                            })}
                        </p>
                    </div>
                    ...
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <div>
                            <CardTitle className="text-lg font-semibold">
                                {t('spend_report.chart_title', {
                                    defaultValue: 'Biểu đồ chi tiêu',
                                })}
                            </CardTitle>
                                ...
                                {percentChange !== 0 && (
                                    <>
                                        ...
                                        <span className="ml-1 text-muted-foreground">
                                            {t('spend_report.vs_previous', {
                                                defaultValue: 'so với hôm qua',
                                            })}
                                        </span>
                                    </>
                                )}
                    ...
                    <CardContent>
                        {barData.length > 0 ? (
                            ...
                            <div className="flex h-[280px] items-center justify-center text-muted-foreground">
                                {t('spend_report.no_chart_data', {
                                    defaultValue: 'Chưa có dữ liệu biểu đồ',
                                })}
                            </div>
                        )}
                    </CardContent>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-lg font-semibold">
                            {t('spend_report.budget_allocation', {
                                defaultValue: 'Phân bổ ngân sách',
                            })}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        {pieData.length > 0 ? (
                            ...
                            <div className="flex h-[200px] items-center justify-center text-muted-foreground">
                                {t('spend_report.no_allocation_data', {
                                    defaultValue: 'Chưa có dữ liệu phân bổ',
                                })}
                            </div>
                        )}
                    </CardContent>
