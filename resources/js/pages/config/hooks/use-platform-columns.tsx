    ...
    return useMemo<ColumnDef<PlatformSetting>[]>(
        () => [
            { accessorKey: 'id', header: t('common.id') },
            {
                ...
                cell: (cell) =>
                    t(platformTypeLabel[cell.row.original.platform]),
            },
            {
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    const value =
                        platform === 2
                            ? cfg.sync_all_accessible_businesses
                                ? t('platform.all_accessible_businesses', {
                                      defaultValue: 'All VIA businesses',
                                  })
                                : cfg.business_manager_id
                    ...
                    return (
                        <div className="flex items-center justify-center gap-2">
                            {onEdit && (
                                <button
                                    type="button"
                                    className="flex h-8 items-center gap-1 rounded border px-3 hover:bg-gray-100"
                                    onClick={() => onEdit(it)}
                                >
                            ...
                            <button
                                type="button"
                                className={`h-8 rounded border px-3 ${disabled ? 'bg-primary text-white' : ''}`}
                                onClick={() =>
                                    router.post(
