...
class NowPaymentsWebhookController
{
    ...
    public function handle(Request $request): JsonResponse
    {
        try {
            ...
            if ($txResult->isError()) {
                Logging::api('NowPaymentsWebhookController@handle: Không tìm thấy giao dịch với payment_id', [
            ...
            if ($paymentStatus === PaymentStatus::CONFIRMED->value || $paymentStatus === PaymentStatus::FINISHED->value) {
                ...
                if ($approveResult->isSuccess()) {
                    Logging::api('NowPaymentsWebhookController@handle: Duyệt nạp tiền thành công', [
                ...
                Logging::error('NowPaymentsWebhookController@handle: Failed to approve deposit', [
                    'payment_id' => $payload['payment_id'],
                    'transaction_id' => $transaction->id,
                ...
                Logging::api('NowPaymentsWebhookController@handle: Thanh toán thất bại/hết hạn', [
                    'payment_id' => $payload['payment_id'],
                    'transaction_id' => $transaction->id,
                ...
                // hanh toán vẫn đang chờ xác nhận
                Logging::api('NowPaymentsWebhookController@handle: Thanh toán đang chờ xác nhận', [
                    'payment_id' => $payload['payment_id'],
                    'transaction_id' => $transaction->id,
                    'status' => $paymentStatus,
                ]);
                ...
                Logging::api('NowPaymentsWebhookController@handle: NowPayments đang chuyển tiền về ví hệ thống', [
                    'payment_id' => $payload['payment_id'],
                    'transaction_id' => $transaction->id,
                    'status' => $paymentStatus,
                ]);
                ...
                Logging::api('NowPaymentsWebhookController@handle: Khách thanh toán thiếu (partially_paid)', [
                    'payment_id' => $payload['payment_id'],
                    'transaction_id' => $transaction->id,
                    'status' => $paymentStatus,
                    'payload' => $payload,
            ...
            Logging::api('NowPaymentsWebhookController@handle: Trạng thái thanh toán không xác định', [
                'payment_id' => $payload['payment_id'],
                'transaction_id' => $transaction->id,
                'status' => $paymentStatus,
                'payload' => $payload,
